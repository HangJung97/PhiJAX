...
def test_heat_equation_quickstart_cli_forwards_runtime_options(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def record_run(max_steps: int, *, accelerator: str, precision: str) -> float:
        ...
        """
        received.update(max_steps=max_steps, accelerator=accelerator, precision=precision)
        return 0.0
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["quickstart.py", "--max-steps", "3", "--accelerator", "auto", "--precision", "bf16-mixed"],
    )

    quickstart.main()

    assert received == {"max_steps": 3, "accelerator": "auto", "precision": "bf16-mixed"}
