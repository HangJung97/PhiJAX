...
class MLP(nnx.Module):
    ...
    def __init__(
        ...
        *,
        ...
        def make_dense(in_features: int, out_features: int) -> nnx.Module:
            """Construct the configured dense-layer implementation.
            ...
            Returns:
                A regular, weight-normalized, or random weight-factorized dense layer.
            """
            ...
            linear = nnx.Linear(in_features, out_features, **linear_kwargs)
            if weight_norm:
                return nnx.WeightNorm(linear, rngs=rngs)
            return linear
        ...
        self.networks = nnx.List(
            [
                nnx.List(
                    [
                        make_dense(in_features, out_features)
...
def initialize_mlp(
    ...
    **model_kwargs: Any,
    ...
    """Construct an MLP and split its static graph from array state.

    Args:
        key: JAX PRNG key for deterministic parameter initialization.
        input_dim: Width of the input vector.
        output_dim: Width of the output vector.
        precision: Optional precision policy supplying model dtypes unless explicitly overridden.
        **model_kwargs: Architecture options forwarded directly to :class:`MLP`.

    Returns:
        NNX graph definition and explicit differentiable model state.

    Raises:
        TypeError: If `model_kwargs` do not match :class:`MLP` arguments.
        ValueError: If :class:`MLP` rejects an architecture option.
    """
    if precision is not None:
        policy = PrecisionPolicy.from_name(precision)
        model_kwargs.setdefault("compute_dtype", policy.compute_dtype)
        model_kwargs.setdefault("parameter_dtype", policy.parameter_dtype)
        model_kwargs.setdefault("output_dtype", policy.output_dtype)
    model = MLP(input_dim, output_dim, rngs=nnx.Rngs(params=key), **model_kwargs)
...
def apply_mlp(
    ...
    *,
    ...
    """Apply a split MLP through an explicit model-state contract.

    Args:
        graphdef: Static NNX graph definition returned by :func:`initialize_mlp`.
        model_state: Explicit NNX parameter state matching `graphdef`.
        inputs: Input array with final width matching the configured `input_dim`.
        input_mean: Optional per-input normalization mean.
        input_std: Optional per-input normalization standard deviation.
        deterministic: Whether to disable configured dropout.
        dropout_key: Explicit PRNG key required for stochastic dropout.
    ...
    Returns:
        Model predictions with unchanged leading input dimensions.
    """
...
def build_mlp(
    ...
    **model_kwargs: Any,
    ...
    Args:
        ...
        **model_kwargs: Architecture options forwarded to :func:`initialize_mlp`.
    ...
    model_summary = partial(
        tabulate_nnx_model,
        graphdef,
        example_inputs=jnp.zeros((1, input_dim), dtype=policy.derivative_dtype),
    )
    return InitializedModel(model_apply, model_state, model_summary)
